...
export default function UserPhotoGallery({ businessId, itineraryId }) {
  ...
  const fetchPhotos = async () => {
    ...
    if (error) console.error('Error fetching user photos:', error);
    else setPhotos(data || []);
    setLoading(false);
  };
      ...
      {selectedPhoto && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={() => setSelectedPhoto(null)}>
          <div className="max-w-3xl max-h-[90vh] bg-white rounded-lg overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <img src={selectedPhoto.image_url} alt="Full size" className="w-full h-auto max-h-[70vh] object-contain" />
            <div className="p-4">
              <p className="text-text">
                {language === 'my' && selectedPhoto.caption_my ? selectedPhoto.caption_my : selectedPhoto.caption || 'No caption'}
              </p>
              <p className="text-text-soft text-sm mt-1">© {getUploaderName(selectedPhoto.user_email)}</p>
            </div>
          </div>
        </div>
      )}
