...
export default function ItineraryList() {
  ...
  const fetchItineraries = async () => {
    const { data, error } = await supabase.from('itineraries').select('*').order('id');
  ...
  const applyFiltersAndSearch = () => {
    ...
    if (filters.duration && filters.duration !== 'all') {
      if (filters.duration === '3+') results = results.filter(i => getDurationDays(i.duration) >= 3);
      else results = results.filter(i => getDurationDays(i.duration) === parseInt(filters.duration));
    }
    const sorts = {
  ...
  return (
    <div className="container-custom">
      <h1 className="page-title">{t('home.title')}</h1>
      <SearchAndFilter type="itinerary" onSearch={setSearchTerm} onFilter={setFilters} onSort={setSortBy} />
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(itinerary => <ItineraryCard key={itinerary.id} itinerary={itinerary} />)}
        </div>
      )}
    </div>
  );
